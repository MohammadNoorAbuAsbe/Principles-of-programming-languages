...
(define pipe$
  (lambda (fs cont)  
    (if (empty? (cdr fs))
        (cont (car fs))
        (compose$ (car fs) (pipe$ (cdr fs) cont) id))))

(define compose$
  (lambda (f$ g$ c1)
    (lambda (x c2)
       (f$ x (lambda (res_f) (g$ res_f c2)))
      )))
;;; Q1.c
...
(define reduce-lzl
  (lambda (reducer init lst)
    (if (empty-lzl? (tail lst))
        (reducer init (head lst))
        (reduce-lzl reducer (reducer init (head lst)) (tail lst)))))
