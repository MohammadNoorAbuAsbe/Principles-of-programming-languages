...
export const parseTraceExp: (params: Sexp[]) => Result<TraceExp> = 
    (params) => 
    isEmpty(params) ? makeFailure("trace missing 1 arguments") :
    ! isEmpty(rest(params)) ? makeFailure("trace has too many arguments") :
    parseGoodTraceExp(first(params)); 

const parseGoodTraceExp = (variable: Sexp): Result<TraceExp> =>
    ! isIdentifier(variable) ? makeFailure("First arg of trace must be an identifier") :
    makeOk(makeTraceExp(makeVarRef(variable)));
